...
target_fw_path="/usr/lib/firmware/updates/qcom/x1p42100/LENOVO/83N3/"
...
partitions=$(lsblk -f -o NAME,FSTYPE | grep -w "ntfs" | grep "nvme" | while read -r name fstype; do echo "/dev/"$(echo ${name} | sed 's/^.*─//;q'); done)
